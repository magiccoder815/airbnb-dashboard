...
export function HostsDataDashboard({ initialData }: HostsDataDashboardProps) {
    ...
    const uniqueYears = [
        ...
        .filter(Boolean)
        .sort((a, b) => a - b);
    const initialYearFilters = Object.fromEntries(
    ...
    const averageRating =
        filteredData.reduce((acc, item) => acc + (item.host.rating || 0), 0) /
            totalHosts || 0;
    const superhostPercentage =
        Math.round(
            (filteredData.filter((item) => item.host.superhost).length /
                totalHosts) *
                100
        ) || 0;
    ...
    const ratingDistribution = [
        {
            ...
            count: filteredData.filter(
                (item) => item.host.rating && item.host.rating < 2
            ).length,
        },
        {
            rating: "2",
        ...
        },
        {
            ...
            count: filteredData.filter(
                (item) =>
                    item.host.rating &&
                    item.host.rating >= 4 &&
                    item.host.rating < 4.5
            ).length,
        },
        {
            rating: "5",
            count: filteredData.filter(
                (item) => item.host.rating && item.host.rating >= 4.5
            ).length,
        },
    ];
    ...
    const yearsData = uniqueYears.map((year) => ({
        year: year.toString(),
        count: filteredData.filter(
            (item) => item.host.started_year && item.host.started_year <= year
        ).length,
    }));
    ...
    useEffect(() => {
        ...
        if (zipCode) {
            result = result.filter((item) =>
                item.zipcode.toString().includes(zipCode)
            );
        }
        // Filter by years
        ...
        if (selectedYears.length > 0) {
            result = result.filter(
                (item) =>
                    ...
                    selectedYears.includes(item.host.started_year)
            );
        }
        ...
        result = result.filter(
            (item) =>
                item.host.rating >= ratingRange[0] &&
                item.host.rating <= ratingRange[1]
        );
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {Object.entries(yearFilters).map(
                                    ([year, isChecked]) => (
                                        <div
                                            key={year}
